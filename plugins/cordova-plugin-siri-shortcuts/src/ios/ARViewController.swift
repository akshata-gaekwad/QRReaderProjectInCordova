...
open class ARViewController: UIViewController, ARTrackingManagerDelegate
{
    ...
    fileprivate var presentValueLabel: UILabel?
    ...
    internal func initializeInternal()
    {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(ARViewController.appWillEnterForeground(_:)), name: NSNotification.Name.NSExtensionHostWillEnterForeground, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(ARViewController.appDidEnterBackground(_:)), name: NSNotification.Name.NSExtensionHostDidEnterBackground, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(self.refreshController(_:)), name: NSNotification.Name(rawValue: "refresh"), object: nil)
        self.initialize()
    }
    
    @objc internal func refreshController(_ notification: Notification)
    {

        let sliderIntValue =  String(format: "%.1f", sliderValue)
        self.presentValueLabel?.text = " \(sliderIntValue) kms"
        initialSetup()
...
//            self.annotations = sortedArray as [AnyObject] as! [ARAnnotation]
    ...
    {
        self.fromLabel?.text = "0 km"
        self.toLabel?.text = "5 kms"
    }
...
//        mapViewController.addAnnotations(self.annotations)
    ...
    func addDebugUi()
    {
        ...
        self.presentValueLabel?.removeFromSuperview()
        self.slider?.removeFromSuperview()
        
        let slider = UISlider()
        slider.backgroundColor = UIColor.darkGray
        slider.frame = CGRect(x: 5, y: self.view.bounds.size.height - 80, width: self.view.bounds.size.width - 10, height: 40)
        slider.layer.cornerRadius = 10.0
        slider.autoresizingMask = [UIView.AutoresizingMask.flexibleWidth, UIView.AutoresizingMask.flexibleTopMargin, UIView.AutoresizingMask.flexibleLeftMargin, UIView.AutoresizingMask.flexibleRightMargin]
        ...
        slider.addTarget(self, action: #selector(ARViewController.changeVlaue(_:)), for: .valueChanged)

        view.addSubview(slider)
        self.slider = slider
        
        let fromLabel = UILabel()
        fromLabel.backgroundColor = UIColor.black
        fromLabel.textColor = UIColor.white
        fromLabel.font = UIFont.boldSystemFont(ofSize: 12)
        fromLabel.frame = CGRect(x: 5, y: self.view.bounds.size.height - 30, width: 40, height: 20)
        fromLabel.numberOfLines = 0
        fromLabel.layer.cornerRadius = 10
        fromLabel.layer.masksToBounds = true
        fromLabel.autoresizingMask = [UIView.AutoresizingMask.flexibleWidth, UIView.AutoresizingMask.flexibleTopMargin, UIView.AutoresizingMask.flexibleLeftMargin, UIView.AutoresizingMask.flexibleRightMargin]
        ...
        backbutton.addTarget(self, action: #selector(ARViewController.backAction), for: .touchUpInside)
        view.addSubview(backbutton)
        self.backButton = backbutton
        
        let toLabel = UILabel()
        toLabel.backgroundColor = UIColor.black
        toLabel.textColor = UIColor.white
        toLabel.font = UIFont.boldSystemFont(ofSize: 12)
        toLabel.frame = CGRect(x: self.view.bounds.size.width - 85, y: self.view.bounds.size.height - 30, width: 80, height: 20)
        toLabel.numberOfLines = 0
        toLabel.layer.cornerRadius = 10
        toLabel.layer.masksToBounds = true
        toLabel.autoresizingMask = [UIView.AutoresizingMask.flexibleWidth, UIView.AutoresizingMask.flexibleTopMargin, UIView.AutoresizingMask.flexibleLeftMargin, UIView.AutoresizingMask.flexibleRightMargin]
        toLabel.textAlignment = NSTextAlignment.center
        view.addSubview(toLabel)
        self.toLabel = toLabel
        
        let presentValueLabel = UILabel()
        presentValueLabel.backgroundColor = UIColor.black
        presentValueLabel.textColor = UIColor.white
        presentValueLabel.font = UIFont.boldSystemFont(ofSize: 12)
        presentValueLabel.frame = CGRect(x: self.view.bounds.size.width / 2 - 50, y: self.view.bounds.size.height - 30, width: 100, height: 20)
        presentValueLabel.numberOfLines = 0
        presentValueLabel.layer.cornerRadius = 10
        presentValueLabel.layer.masksToBounds = true
        presentValueLabel.autoresizingMask = [UIView.AutoresizingMask.flexibleWidth, UIView.AutoresizingMask.flexibleTopMargin, UIView.AutoresizingMask.flexibleLeftMargin, UIView.AutoresizingMask.flexibleRightMargin]
        presentValueLabel.textAlignment = NSTextAlignment.center
        presentValueLabel.text = "2.5 kms"
        view.addSubview(presentValueLabel)
        self.presentValueLabel = presentValueLabel
    ...
    @objc func changeVlaue(_ slider : UISlider){
        
        sliderValue = slider.value
        
        let imageDataDict:[String: Float] = ["value": sliderValue]
        
        // post a notification
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "slidervaluechanged"), object: nil, userInfo: imageDataDict)
