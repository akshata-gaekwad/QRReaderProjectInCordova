...
final class StartViewController:UIViewController,  Controller {
    ...
    func showARViewController(){
        ...
        arViewController.maxDistance = 500000
    ...
    private func setupNavigation() {
        ...
        DispatchQueue.global(qos: .default).async {
            ...
            if self.destinationLocation != nil {
                self.navigationService.getDirections(destinationLocation: self.destinationLocation, request: MKDirectionsRequest()) { steps , error in
...
//        addMapAnnotations()
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            SVProgressHUD.dismiss()
            let alert = UIAlertController(title: self.placeName , message: self.infoText, preferredStyle: UIAlertControllerStyle.alert)
